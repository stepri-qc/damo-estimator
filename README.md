...
On the Netlify deployment, **Extract with AI now** in the "Import Claude-prepared inputs" section sends `combinedIntakeText()` straight to a real backend — `netlify/functions/extract-inputs.mts` — which calls the Claude API (`claude-sonnet-5`) and returns the result directly into the page. No copy-paste, no second chat window.
...
- **The function streams its response — confirmed necessary, not precautionary.** A real 23-page/~32K-character RFP tripped Netlify's proxy inactivity timeout on the first version of this function (504 "Inactivity Timeout... too much time has passed without sending any data" at ~30s) — that's an *idle-connection* timeout, not a hard duration cap, because the function buffered the entire Claude response before sending anything back. The function now streams: plain keep-alive bytes flow to the client for every text delta Claude produces, keeping the connection visibly active, followed by one `__DAMO_RESULT__`-prefixed JSON payload once generation finishes. The frontend reads the response body incrementally, shows a live "bytes received" counter while waiting, and parses only what follows the marker — everything before it is discarded filler, never meant to be parsed.
