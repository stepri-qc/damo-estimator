...
**Why it works the way it does.** A published Artifact is a static page with no backend and no runtime LLM access — checked directly against the platform's own capability list, which covers only file-download and connector calls, not summarization. So "reads the document and creates a summary" cannot mean a live AI call from inside the page itself. There is also hard evidence from this build that scripted downloads/popups/print are unreliable in the real deployment sandbox (see §9). Given both constraints, the intake screen is built from three paths:
